...
sys.stdout = open(f"./results/console_output_lr{learning_rate}_bs{batchsize}_inference_time.txt", "w")
...
def train_test_plot(method_name = "_"):
    ...
    avg_inference_time = np.mean(inference_times) / len(testloader.dataset)
    print(f'Average Inference Time per Image: {avg_inference_time:.10f} seconds')
